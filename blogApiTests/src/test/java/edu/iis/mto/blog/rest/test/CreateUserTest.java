...
public class CreateUserTest extends FunctionalTests {
    ...
    public void createUserShouldRequireUniqueEmail() {
        JSONObject jsonObj = new JSONObject().put("email", "khaled@domain.com")
        		.put("firstName", "Khaled")
        		.put("lastname", "Wielki");
        RestAssured.given().accept(ContentType.JSON).header("Content-Type", "application/json;charset=UTF-8")
                .body(jsonObj.toString()).expect().log().all().statusCode(HttpStatus.SC_CREATED).when()
                .post("/blog/user");
        
        JSONObject sameJsonObj = new JSONObject().put("email", "khaled@domain.com")
		        .put("firstName", "Delahk")
				.put("lastname", "Wielki");
        RestAssured.given().accept(ContentType.JSON).header("Content-Type", "application/json;charset=UTF-8")
